...
class DashboardDataRepository implements DashboardRepository {
  ...
  Future<Either<Failure, DummyGetOtpDtoEntity>> sendOTP(String number) async {
    try {
      final response = await _apiService.post(
          "https://karmasangsthan.com.bd/api/app_api/auth/get-otp",
          body: {'phone': number});
  ...
  @override
  Future<Either<Failure, DummyVerifyOtpDTOEntity>> verifyOTP(
      String userId, String otp) async {
    try {
      final response = await _apiService.post(
          "https://karmasangsthan.com.bd/api/app_api/auth/login",
          body: {'user_id': userId, 'otp': otp});
      final data = DummyVerifyOtpDTOEntity.fromJson(jsonDecode(response.body));
      return Right(data);
    } catch (error) {
      if (error is NetworkException) {
        return Left(NetworkFailure(error.message));
